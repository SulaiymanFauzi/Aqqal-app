...
import { Platform, StyleSheet, View as RNView, Linking, Text as RNText, Alert, Animated, Easing, Image, Pressable, Dimensions, ScrollView } from 'react-native';
...
function massageContent(input: string): string {
  ...
  let s = input.replace(/\r\n/g, '\n');
  // Remove bullets starting with * or - at line starts (strip them completely)
  s = s.replace(/(^|\n)(\s*)[\*\-]\s+/g, '$1');
  // Blockquotes starting with > at line starts (support no-space after >)
  s = s.replace(/(^|\n)\s*>\s*/g, '$1▎ ');
  // Remove leading spaces from paragraphs (but preserve newlines and blockquotes)
  s = s.replace(/\n[ \t]+(?![▎\n])/g, '\n');
  // Trim trailing whitespace to prevent empty paragraphs at the end
  s = s.trimEnd();
  return s;
}

function isBulletParagraph(segments: Segment[]): boolean {
  if (segments.length === 0) return false;
  const firstSeg = segments[0];
  return firstSeg.type === 'text' && /^\s*[•]/.test(firstSeg.text ?? '');
}

function expandQuotedHighlights(segments: Segment[]): Segment[] {
  const backtickPattern = /`([^`]+?)`/g; // backtick-wrapped terms
  const expanded: Segment[] = [];

  for (const seg of segments) {
    // Skip segments that shouldn't contain backticks
    if (seg.type === 'link' || seg.type === 'heading' || seg.type === 'mark') {
      expanded.push(seg);
      continue;
    }
    ...
    while ((match = backtickPattern.exec(text))) {
      ...
      if (start > lastIndex) {
        expanded.push({ ...seg, text: text.slice(lastIndex, start) });
      }

      // Split long terms at spaces to allow wrapping
      const words = trimmedInner.split(/(\s+)/);
      words.forEach((word, idx) => {
        if (word.length > 0) {
          if (/\s/.test(word)) {
            // Preserve spaces with parent segment type
            expanded.push({ ...seg, text: word });
          } else {
            // Each word gets marked individually
            expanded.push({ type: 'mark', text: word });
          }
        }
      });
    ...
    // If we didn't match any backtick term, keep segment as-is
    if (matchCount === 0) {
      expanded.push(seg);
      continue;
    }
    ...
    if (lastIndex < text.length) {
      expanded.push({ ...seg, text: text.slice(lastIndex) });
    }
  }
...
export default function ChatMessage({ msg, onLayout }: Props) {
  ...
  React.useEffect(() => {
    if (!upcomingSummary) {
      if (isStreaming) {
        ...
        // When finished streaming, show "Finished thinking" instead of hiding
        ...
        if (summaryRef.current !== finishedText) {
          ...
          fadeAnim.setValue(1); // Keep it visible
        }
      }
      return;
    }
  ...
  // Trigger replacement animation when shouldAnimateReplacement is set
  React.useEffect(() => {
  ...
  }, [isUser, msg.content, streamingOpacity, replacedOpacity]);
  ...
  // Check if text is a standalone Arabic paragraph (not inline)
  const isStandaloneArabic = React.useCallback(
    (segments: Segment[]): boolean => {
      // Check if all segments are text/link/italic (common in verses)
      const allText = segments.every(seg => seg.type === 'text' || seg.type === 'link' || seg.type === 'italic');
      ...
      // Check if any line is purely Arabic (verses/hadiths often have translation after)
      const lines = combinedText.split('\n').map(l => l.trim()).filter(l => l.length > 0);
      let hasArabicLine = false;
      
      for (const line of lines) {
        // Skip lines that are clearly references or translations
        if (line.startsWith('[') || line.startsWith('"') || line.startsWith('*') || line.startsWith('O ')) {
          continue;
        }
        
        const cleanLine = line.replace(/[\s.,;:!?،؛\[\]\(\)""''*]/g, '');
        const isLineArabic = cleanLine.length > 10 && 
          /^[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]+$/.test(cleanLine);
      ...
      // Fallback: check if entire paragraph is Arabic
      const cleanText = combinedText.replace(/[\s\n.,;:!?،؛\[\]\(\)""''*]/g, '');
      const isAllArabic = cleanText.length > 0 && /^[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]+$/.test(cleanText);
  ...
  const renderTextWithArabic = React.useCallback(
    (value: string, key: string, baseStyle?: StyleProp<TextStyle>, useSpecialFont: boolean = false) => {
      ...
      while ((match = arabicRun.exec(value))) {
        ...
        // Check if this Arabic text contains ﷺ (U+FDFA) or similar honorific symbols
        const arabicText = match[0];
        const hasHonorific = /[\uFDFA\uFDFB]/.test(arabicText); // ﷺ and ﷻ
        
        nodes.push(
          <RNText
            key={`${key}-arabic-${runIndex++}`}
            style={useSpecialFont && !hasHonorific ? [baseStyle, styles.arabicText] : baseStyle}
          >
            {arabicText}
          </RNText>
        );
  ...
  const renderInlineSegments = React.useCallback(
    (list: Segment[], keyPrefix: string, parentStyle?: StyleProp<TextStyle>, useSpecialFont: boolean = false): (string | React.ReactNode)[] =>
      list.map((seg, idx) => {
        const key = `${keyPrefix}-${idx}`;
        const baseStyle: StyleProp<TextStyle> = [parentStyle];
        if (seg.type === 'text') {
          if (isArabic(seg.text)) {
            return renderTextWithArabic(seg.text, key, baseStyle, useSpecialFont);
          }
          return (
  ...
  const renderBulletOrParagraph = React.useCallback(
    (paragraphSegments: Segment[], key: string, baseParagraphStyle: StyleProp<TextStyle>, applySpacing: boolean, noMargin?: boolean) => {
      // Check if this is a standalone Arabic paragraph
      const useSpecialFont = isStandaloneArabic(paragraphSegments);
      ...
      return (
        ...
        >
          {renderInlineSegments(paragraphSegments, key, baseParagraphStyle, useSpecialFont)}
        </Text>
      );
    },
    [cardTone.textColor, isUser, renderInlineSegments, isStandaloneArabic]
  );
        ...
        {msg.attachments && msg.attachments.length > 0 && (
          ...
          >
            {msg.attachments.map((attachment) => (
              <RNView key={attachment.id} style={styles.attachmentWrapper}>
                <Image
        ...
        >
          ...
          {isUser ? (
            paragraphs.map((paragraphSegments, idx) => {
              ...
              return renderBulletOrParagraph(
                paragraphSegments,
                `paragraph-${idx}`,
                baseParagraphStyle,
            ...
            (() => {
              // Process the full content together to maintain proper paragraph structure
              const fullContent = stablePrefix + fadeSuffix;
              const allSegments = segmentsToParagraphs(parseContent(massageContent(fullContent)));
              const stableLength = stablePrefix.length;
              
              return (
                <>
                  {allSegments.map((paragraphSegments, idx) => {
                    const baseParagraphStyle: StyleProp<TextStyle> = [{ color: cardTone.textColor }];
                    
                    // Determine if this paragraph contains any fading content
                    let charCount = 0;
                    const hasFadingContent = paragraphSegments.some(seg => {
                      const segStart = charCount;
                      charCount += seg.text?.length ?? 0;
                      return segStart < fullContent.length && charCount > stableLength;
                    });
                    
                    if (hasFadingContent && fadeSuffix) {
                      // This paragraph has fading content - wrap in animated view
                      return (
                        <Animated.View key={`paragraph-${idx}`} style={{ opacity: streamingOpacity }}>
                          {renderBulletOrParagraph(
                            paragraphSegments,
                            `paragraph-${idx}`,
                            baseParagraphStyle,
...
const styles = StyleSheet.create({
  ...
  arabicText: {
    ...
    lineHeight: Platform.OS === 'web' ? 50 : 64,
  ...
  markOnTint: {
    ...
    borderBottomColor: 'rgba(35,49,47,0.3)',
  },
  link: {
